        ...
        public static class Exists
        {
            ...
            public static void ThrowsExceptionIfMatchIsNull()
                => Assert.Throws<ArgumentNullException>(
                    () => new Matrix4x4().Exists(match: null as Predicate<float>));

            [TestCaseSource(typeof(Exists), nameof(PredicateTestCases))]
            public static bool Test(Matrix4x4 matrix, Predicate<float> match)
                => matrix.Exists(match);

            [TestCaseSource(typeof(Exists), nameof(IPredicateTestCases))]
            public static bool Test<FloatPredicate>(Matrix4x4 matrix, FloatPredicate match)
                where FloatPredicate : struct, IPredicate<float>
                => matrix.Exists<float, Matrix4x4, FloatPredicate>(match);
