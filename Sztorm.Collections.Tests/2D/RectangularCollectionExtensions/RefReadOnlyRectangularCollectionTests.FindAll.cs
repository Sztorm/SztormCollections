    ...
    public partial class RefReadOnlyRectangularCollectionTests
    {
        public static class FindAll
        {
            ...
            public static class Indices
            {
                ...
                public static void ThrowsExceptionIfMatchIsNull()
                    => Assert.Throws<ArgumentNullException>(
                        () => new RefReadOnlyMatrix4x4().FindAllIndices<
                            float, List<Index2D>, RefReadOnlyMatrix4x4>(match: null));

                [TestCaseSource(typeof(FindAll), nameof(IndicesTestCases))]
                public static void TestFindAll<Index2DCollection>(
                    RefReadOnlyMatrix4x4 matrix, Predicate<float> match, Index2DCollection expected)
                    where Index2DCollection : ICollection<Index2D>, new()
                    => CollectionAssert.AreEqual(expected, matrix.FindAllIndices<
                        float, Index2DCollection, RefReadOnlyMatrix4x4>(match));
