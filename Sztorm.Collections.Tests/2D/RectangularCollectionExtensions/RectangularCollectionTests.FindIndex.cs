        ...
        public static class FindIndex
        {
            public static class Predicate
            {
                ...
                public static void ThrowsExceptionIfMatchIsNull()
                    => Assert.Throws<ArgumentNullException>(
                        () => new Matrix4x4().FindIndex<float, Matrix4x4>(match: null));
                ...
                public static void Index2DBounds2DThrowsExceptionIfMatchIsNull()
                    => Assert.Throws<ArgumentNullException>(
                        () => new Matrix4x4().FindIndex<float, Matrix4x4>(
                ...
                public static void Index2DBounds2DThrowsExceptionIfStartIndexIsOutOfBounds(
                    Matrix4x4 matrix, Index2D startIndex)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => matrix.FindIndex<float, Matrix4x4>(
                ...
                public static void Index2DBounds2DThrowsExceptionIfSectorIsOutOfBounds(
                    Matrix4x4 matrix, Index2D startIndex, Bounds2D sector)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => matrix.FindIndex<float, Matrix4x4>(startIndex, sector, o => true));

                [TestCaseSource(typeof(FindIndex), nameof(PredicateIndex2DBounds2DTestCases))]
                public static ItemRequestResult<Index2D> Test(
                    Matrix4x4 matrix, Index2D startIndex, Bounds2D sector, Predicate<float> match)
            ...
            public static class IPredicate
            {
                ...
                public static ItemRequestResult<Index2D> Test<FloatPredicate>(
                    ...
                    => matrix.FindIndex<float, Matrix4x4, FloatPredicate>(match);

                [TestCaseSource(typeof(FindIndex), nameof(InvalidStartIndexTestCases))]
                public static void Index2DBounds2DThrowsExceptionIfStartIndexIsOutOfBounds(
                    Matrix4x4 matrix, Index2D startIndex)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => matrix.FindIndex<float, Matrix4x4, AlwaysTruePredicate<float>>(
                ...
                public static ItemRequestResult<Index2D> Test<FloatPredicate>(
                    Matrix4x4 matrix, Index2D startIndex, Bounds2D sector, FloatPredicate match)
                    where FloatPredicate : struct, IPredicate<float>
                    => matrix.FindIndex<float, Matrix4x4, FloatPredicate>(
                        startIndex, sector, match);
            }

            private static IEnumerable<TestCaseData> InvalidStartIndexTestCases()
            {
                yield return new TestCaseData(new Matrix4x4(), new Index2D(-1, 0));
                yield return new TestCaseData(new Matrix4x4(), new Index2D(0, -1));
                yield return new TestCaseData(new Matrix4x4(), new Index2D(4, 0));
                yield return new TestCaseData(new Matrix4x4(), new Index2D(0, 4));
            }

            private static IEnumerable<TestCaseData> InvalidSectorTestCases()
            {
                yield return new TestCaseData(
                    new Matrix4x4(), new Index2D(0, 0), new Bounds2D(5, 0));
                yield return new TestCaseData(
                    new Matrix4x4(), new Index2D(0, 0), new Bounds2D(0, 5));
                yield return new TestCaseData(
                    new Matrix4x4(), new Index2D(1, 1), new Bounds2D(4, 0));
                yield return new TestCaseData(
                    new Matrix4x4(), new Index2D(1, 1), new Bounds2D(0, 4));
            }
