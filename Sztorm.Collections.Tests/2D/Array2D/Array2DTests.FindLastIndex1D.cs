...
namespace Sztorm.Collections.Tests
{
    ...
    {
        public static class FindLastIndex1D
        {
            public static class Predicate
            {
                ...
                public static void ThrowsExceptionIfMatchIsNull()
                    => Assert.Throws<ArgumentNullException>(
                        () => new Array2D<int>(0, 0).FindLastIndex1D(match: null));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(PredicateTestCases))]
                public static ItemRequestResult<int> Test<T>(
                    Array2D<T> array, Predicate<T> match)
                    => array.FindLastIndex1D(match);

                [Test]
                public static void Index2DIntThrowsExceptionIfMatchIsNull()
                    => Assert.Throws<ArgumentNullException>(
                        () => new Array2D<int>(0, 0).FindLastIndex1D((0, 0), 0, match: null));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(InvalidStartIndexCases))]
                public static void Index2DIntThrowsExceptionIfStartIndexIsOutOfBounds<T>(
                    Array2D<T> array, Index2D startIndex)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => array.FindLastIndex1D(startIndex, 0, o => true));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(InvalidCountTestCases))]
                public static void Index2DIntThrowsExceptionIfCountExceedsArray2DCount<T>(
                    Array2D<T> array, Index2D startIndex, int count)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => array.FindLastIndex1D(startIndex, count, o => true));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(PredicateIndex2DIntTestCases))]
                public static ItemRequestResult<int> Test<T>(
                    Array2D<T> array, Index2D startIndex, int count, Predicate<T> match)
                    => array.FindLastIndex1D(startIndex, count, match);

                [Test]
                public static void Index2DBounds2DThrowsExceptionIfMatchIsNull()
                    => Assert.Throws<ArgumentNullException>(
                        () => new Array2D<int>(0, 0).FindLastIndex1D((0, 0), new Bounds2D(), match: null));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(InvalidStartIndexCases))]
                public static void Index2DBounds2DThrowsExceptionIfStartIndexIsOutOfBounds<T>(
                    Array2D<T> array, Index2D startIndex)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => array.FindLastIndex1D(startIndex, new Bounds2D(), o => true));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(InvalidSectorTestCases))]
                public static void Index2DBounds2DThrowsExceptionIfSectorIsOutOfBounds<T>(
                    ...
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => array.FindLastIndex1D(startIndex, sector, o => true));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(PredicateIndex2DBounds2DTestCases))]
                public static ItemRequestResult<int> TestFindLastIndex1D<T>(
                    Array2D<T> array, Index2D startIndex, Bounds2D sector, Predicate<T> match)
                    => array.FindLastIndex1D(startIndex, sector, match);
            }

            public static class IPredicate
            {
                [TestCaseSource(typeof(FindLastIndex1D), nameof(IPredicateTestCases))]
                ...
                public static void Index2DIntIPredicateThrowsExceptionIfStartIndexIsOutOfBounds<T>
                    (Array2D<T> array, Index2D startIndex)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => array.FindLastIndex1D(startIndex, 0, new AlwaysTruePredicate<T>()));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(InvalidCountTestCases))]
                public static void Index2DIntIPredicateThrowsExceptionIfCountExceedsArray2DCount<T>
                    (Array2D<T> array, Index2D startIndex, int count)
                    => Assert.Throws<ArgumentOutOfRangeException>(
                        () => array.FindLastIndex1D(
                            startIndex, count, new AlwaysTruePredicate<T>()));

                [TestCaseSource(typeof(FindLastIndex1D), nameof(Index2DIntIPredicateTestCases))]
                public static ItemRequestResult<int> Test<T, TPredicate>(
                ...
                [TestCaseSource(typeof(FindLastIndex1D), nameof(Index2DBounds2DIPredicateTestCases))]
                public static ItemRequestResult<int> Test<T, TPredicate>(
            ...
            private static IEnumerable<TestCaseData> PredicateTestCases()
            {
                ...
                yield return new TestCaseData(array3x3, new Predicate<int>(o => o > 5))
                    .Returns(new ItemRequestResult<int>(6));
